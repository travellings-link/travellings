...
### Q：我放置了开往，但是被误报为"LOST"
A：
可能原因 -> 您的网站需要下滑至某个高度或鼠标放置到某个位置，才能看到开往。
解决方案 -> 请在醒目位置放置开往链接

可能原因 -> 您的开往链接未正确填写，正确的填写：<https://www.travellings.cn> 或 <www.travellings.cn> ，错误的填写： <https://travellings.xiaozongling.cn>（临时站点）或<//www.travellings.cn>。此种状态下不属于误报。
解决方案 -> 请正确填写开往链接

可能原因 -> 您的网站使用特殊框架，主页面中存在开往图标但不是常规的标签跳转的、首页有强制非自动关闭弹窗。
解决方案 -> 我们定期为这部分博客专门适配
